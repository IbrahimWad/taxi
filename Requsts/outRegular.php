...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    empty($_POST['phone']) || empty($_POST['salary'])|| empty($_POST['payment'])) {
        # code...
        ...
        // Insert new user into database
            ...
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM outregular WHERE phone = ?');
            $stmt->execute([$phone]);
            ...
            if ($stmt->fetchColumn() > 0) {
                ...
                $stmt = $pdo->prepare('INSERT INTO outregular (mylatitude, mylongitude, latitude, longitude, phone, salary, payment) 
